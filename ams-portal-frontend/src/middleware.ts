...
export async function middleware(req: NextRequest) {
  ...
  try {
    if (token) {
      response.headers.set("X--ID", token);
      (req as AuthenticatedRequest).user = { id: token };
    }
  } catch (error) {
...
export const config = {
  matcher: ["/dashboard", "/dashboard/upload", "/dashboard/chat", "/login", "/api/users/:path*", "/api/auth/logout"],
};